...
Excel.Application.WorkBooks.Add('таблица.xls');
Excel.DisplayAlerts:=False;
with StringGrid1 do
  for i:=1 to RowCount-1 do
  for j:=1 to ColCount-1 do
    Cells[j, i]:=Excel.WorkSheets.Item['Лист1'].Cells[i, j];
    Excel.ActiveWorkbook.SaveAs('Имя_Файла');  }
...
b:=ln(abs(eint/emax))/k;
if b<-2.5 then b:=-2.5 ;
b:=b*abs(eint)/eint+2.5;
...
begin
  ...
  begin
    ...
    y:=self._charges[counter]._pos.y+dropout*sin(angle);
      ...
      eint:=0;
         ...
         begin
          ...
          if (r<>0) and (self._charges[counter]._q>0)   then       e:=self._charges[counter2]._q/sqr(r) else  e:=-1*self._charges[counter2]._q/sqr(r) ;
          ex:=ex+(e)*cosa;
          ey:=ey+(e)*sina;
         end;
    ...
    // if eint<>0 then    linecolor:=rgb(255,0,round(255*abs(ex/eint)));
...
var
scalar,a,b,ex,ey,e,emax,emin,eint,x,y,r,cosa,sina,angle:real;
i,counter,counter1,counter2,curve:integer; //0-charge,1-line,2-influence
linecolor:tcolor;
begin
for counter := 0 to self._cntcharges-1  do     //cycle which cycles through charges
begin
  for counter1 :=0   to 100 do    //through different angles of start
  begin
    curve:=0;    //crutch  for length
    angle:=(counter1/100)*2*pi;
    ...
    y:=self._charges[counter]._pos.y+10*sin(angle);
      ...
      eint:=0;
         ...
         eint:=(sqr(ex)+sqr(ey)); //summary
         if eint<>0 then         x:=x+4*ex/eint;
         if eint<>0 then      y:=y+4*ey/eint;
         linecolor:=myspace.colour(eint);
         self._canvas.Pixels[round(x),round(y)]:=linecolor;
         curve:=curve+1;
         end;
       end;
